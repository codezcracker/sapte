...
const BROTLI_OPTS = {
  params: {
    [zlib__default['default'].constants.BROTLI_PARAM_QUALITY]: zlib__default['default'].constants.BROTLI_MAX_QUALITY
  }
};
/**
...
class SizePlugin {
  ...
  async apply(compiler) {
    ...
    const done = (stats, callback) => {
      this.outputSizes(stats.compilation.assets).then(output => {
  ...
  async outputSizes(assets) {
    ...
    const sizes = await Promise.all(assetNames.map(name => this.getCompressedSize(assets[name].source()))); // map of de-hashed filenames to their final size
  ...
  async getSizes(cwd) {
    ...
    const sizes = await Promise.all(files.map(async file => {
      const source = await lib.promises.readFile(path__default['default'].join(cwd, file)).catch(() => null);
  ...
  async getCompressedSize(source) {
    ...
    if (this.compression === 'gzip') {
      const gz = promisify__default['default'](zlib__default['default'].gzip);
      compressed = await gz(source, GZIP_OPTS);
    } else if (this.compression === 'brotli') {
      if (!zlib__default['default'].brotliCompress) throw Error('Brotli not supported in this Node version.');
      const br = promisify__default['default'](zlib__default['default'].brotliCompress);
